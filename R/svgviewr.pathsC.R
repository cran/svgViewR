svgviewr.pathsC <- function(path, file=NULL, col=NULL, col.fill="none", col.stroke = "black", z.index = 0, 
	...
	for(i in 1:length(path)){
		...
		new_lines[i] <- paste("\t<pathC z-index=\"", z.index[i], "\" layer=\"", layer[i], "\" label=\"", label[i], "\" stroke=\"", col.stroke[i], "\" stroke-width=\"", lwd[i], "\" stroke-opacity=\"", opacity.stroke[i], "\" fill=\"", col.fill[i], "\" fill-opacity=\"", opacity.fill[i], "\" d=\"", paste(path[[i]] - 1, collapse=" "), "\" />", sep="")
	}
